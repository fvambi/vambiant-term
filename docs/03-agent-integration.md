...
**As built in M3 (2026-09-05, bare CLI path):** the daemon's `http` hook receiver holds the `PermissionRequest` connection for up to 90 s (well under the 600 s hook timeout) while the inbox waits for a human; an inbox decision is returned as the vendor-shaped `allow`/`deny` JSON, so the agent never shows its own prompt. If nobody answers in time the receiver returns `{}`, the agent's own prompt appears in its terminal, and the inbox item stays with `prompt_shown = true`; answering it then injects `y`/`n` into the session's PTY — a labelled, second-best path. A watchdog thread re-notifies every 120 s while an item is unanswered (`reminded ×N` in `vterm inbox list`) and withdraws items the user answered in the agent's own prompt (Codex reports this as `serverRequest/resolved`). `vterm inbox edit` allows with `updatedInput` (Claude Code only). Sessions whose adapter goes quiet are labelled: a Claude session with no hook event for 30 s, or a Codex session whose app-server cannot be reached, carries a `degraded` reason that `vterm ls` prints verbatim. For headless runs (`claude -p --permission-prompts none`) the hook is the *only* approval surface, which is exactly what makes `vterm new --agent claude -- claude -p …` supervisable.
...
**How the daemon uses it (implemented in M3, verified 2026-09-05).** Every Codex session gets its own `codex app-server --listen unix://<runtime>/codex-<id>.sock`, started detached like the fd holder. The user's TUI runs in the session PTY as `codex --remote unix://<that socket> [user args]`, so the user's experience is untouched. `vtermd` joins the same server as a second client, calls `thread/loaded/list`, and `thread/resume`s every thread it sees (retrying while the server still says "no rollout found"). From then on it receives the full turn stream — items, token usage, thread status — and the approval requests themselves. An approval becomes an inbox item; `vterm inbox allow|deny` answers it as the JSON-RPC response on the daemon's connection, and if the user answers in the TUI first the server's `serverRequest/resolved` withdraws the item (recorded as `approval_resolved` by `agent`). No hooks, no profile, no config mutation: the app-server reads the user's own `~/.codex/config.toml`, and sandbox or approval flags given on the `codex` command line reach the thread the TUI starts. Tests stand in a scripted fake app-server for the real one (`crates/vtermd/tests/end_to_end.rs`).
...
**As built (2026-09-05):** the session thread runs a detector for generic sessions. Mode transitions come from scanning the byte stream (`?1049`, `?47`, `?2004`, kitty push/pop) and are logged as `terminal-mode` notifications; after 700 ms without output the last six screen lines are matched against the prompt packs — the built-in `crates/vt-agent/packs/generic.json` plus any `<state>/packs/*.json` — and an OSC 133 mark on the last used row wins over everything (shell prompt ⇒ idle). Each verdict is recorded as a `guess` notification carrying the evidence ("looks like a question (generic/yes-no-paren): Apply changes? (y/N)") and sets the session state; a recognised question raises a desktop notification once per distinct text. Generic sessions carry a permanent `degraded` label saying the state is a guess and approvals cannot be routed. Packs are data: a broken pack is skipped with a message, never fatal. Only the Claude Code and Codex TUI prompt forms are verified against real screens; no other agent CLI was available to capture.
