...
> **M5 status 2026-09-06:** the block engine and shell integration are built and tested (`vt-core` OSC scanner, `vt-blocks` segmenter, `vt-store` persistence, daemon `session.block`/`session.blocks`, `vterm blocks`, `vt-shell` snippets + injection wired into the daemon). Still to build for the exit criterion, all GUI-side: block and agent-event rendering in the app timeline, the cost/context meters, the sidebar tree, and the inbox sheet.
