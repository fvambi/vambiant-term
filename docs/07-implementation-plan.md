...
> **M5 status 2026-09-06:** the block engine is built and tested (`vt-core` OSC scanner, `vt-blocks` segmenter, `vt-store` persistence, daemon `session.block`/`session.blocks`, `vterm blocks`). Still to build for the exit criterion: the shell-integration snippets and their auto-injection, block/agent-event rendering in the app, the cost/context meters, the sidebar tree, and the inbox sheet. Those are GUI-side and land next.
