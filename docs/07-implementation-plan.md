...
- [x] Route table, fallback chains, circuit breakers, ~~cancellation~~. *(`providers.toml` profiles, models, pricing and routes; `config.toml [ai.routes]` wins. 2026-09-06: `fallback` per profile, jittered backoff on 5xx, a 60 s breaker on 429, `vterm ai doctor` shows both; docs/04 §8 as-built. Cancellation not yet.)*
- [x] Cost accounting, budgets with hard stop, `vterm ai spend`. *(2026-09-06: costs and sessions in the egress log, `ai.spend`, the hard stop before anything leaves, the 80 %/100 % banner in the app; docs/04 §7 as-built.)* *(List-price estimates per request, recorded in the egress log; budgets and `spend` not yet.)*
...
- [~] `vt-redact`: gitleaks rule import into a `RegexSet`, entropy layer, streaming sliding window, stateful multi-line PEM mode. *(Entropy layer built 2026-09-06 — docs/05 §3.3 as-built; JSON-style assignments too. Streaming window and streaming PEM still not needed: every payload is assembled before it leaves.)* *(2026-09-06: the rule set (14 credential shapes written from their public formats, in a `RegexSet` plus per-rule regexes), whole-payload PEM, a time budget and panic containment that fail closed, with a golden test per rule. Entropy, the streaming window and streaming PEM are still placeholders.)*
