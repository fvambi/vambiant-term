...
| B5 error underline | **Built 2026-09-06** (daemon `path.executables` from a minute-cached PATH scan; the editor dashes the first word in the theme's red when it is not on PATH, a builtin/keyword, a path or an assignment; aliases and functions are not visible yet and underline too, which the hint calls a guess). First token not found on the session's `PATH` (daemon `which` cache, refreshed on `PATH` change via OSC 7-style env report or `hash -r` heuristic) → dashed red underline decoration; suppressed for shell builtins/aliases/functions (`vt-shell` snippet exports the alias/function list once per prompt via a private OSC) | e2e: unknown command underlined, `ls` not | `10 §10` new OSC documented |
| B8 corrections | **Built 2026-09-06** as `vt_blocks::correct`: rule families as Rust data tables (not TOML files) after thefuck's (MIT, credited) — output-named fixes (`git push --set-upstream`, "most similar command", "Did you mean"), `sudo`/`chmod +x`, git/cargo/docker/npm subcommand typos, `cd..`/fuzzy `cd`, `mkdir -p`, `rm -r`, `python3`, misspelled executables over PATH by optimal-string-alignment distance; daemon `correct.suggest {session}`; the app shows the first as ghost text in the empty editor with the reason in the hint, → accepts, ⎋ drops. e2e: `gti status` in zsh → `git status`. `vt-corrections` module in `vt-blocks`: rule families as data (`rules/*.toml` ported from thefuck, MIT, credited): `git` (typo subcommands, `--set-upstream`), `cd` (fuzzy path), `chmod +x`, `sudo`, `brew`, `npm`, `pip`, `cargo`, `docker`, `mkdir -p`, generic misspelled executable (edit distance over `PATH`); on non-zero exit compute ≤1 correction; app shows it in the failure affordance (`06 §4`) next to "explain"; accept inserts into the input | rule tests per family, corpus from thefuck's tests rewritten (no code copied) | `06 §4`, licence note in `10` |
...
| B11 pane sync | **Built 2026-09-06** (`⌘⌥I` toggles the pane; typed lines and keys go to every other synced pane in the window; `⇄` in the sidebar and a hint line; no cross-tab group yet). App-side fan-out: `syncGroup` on the window; typed bytes go to every member; tab indicator; `⌘⌥I` toggles current tab, palette action for all tabs | Swift: group membership | `06 §7` |
| B12 workflows | **Built 2026-09-06**: `vt-workflows` reads the four directories (user, `~/.warp/workflows`, repo `.vambiant-term/workflows`, repo `.warp/workflows`), `workflows.list {cwd}`, `vterm workflow list|show --arg k=v`, palette scope `w:` with an argument sheet that stages the rendered command; undeclared `{{placeholders}}` become arguments. Not yet: `⇧Tab` cycling in the input, "Save as workflow" from a block. `vt-workflows`: YAML `{name, command, description, tags, arguments:[{name, description, default_value, enum?}], shells}` read from `~/.config/vambiant-term/workflows/` and `<repo>/.vambiant-term/workflows/` (and, read-only, `.warp/workflows/` for drop-in compatibility); `{{arg}}` substitution; `⇧Tab` cycles argument fields in the input; "Save as workflow" from a block | parse/substitute tests, e2e insert | `09`, `02` |
...
| Item | Mechanism | Tests | Docs |
|---|---|---|---|
| C1 tabs | Rename built 2026-09-06 (`tab.rename`, Shell → Rename Tab…, through `session.rename`; `⌘⇧T` reopen too). `⌘1..9`, `⌘⇧{ }`, rename (double-click title or `tab.rename`), `[window] new_tab_placement`; reopen closed = reattach the most recently detached session (`session.list` sorted by detach time), `⌘⇧T` | Swift keymap tests | `06 §7`, `09` |
...
| C9 restore | **Partly built 2026-09-06**: on launch every running daemon session is reattached as a tab, oldest first (blocks come from the daemon); the per-window layout snapshot (splits) is not. M9: `vt-store` layout snapshot per window on change; on launch reattach; blocks come from the daemon | e2e restart | `07` M9 |
...
| C14 quit | **Built 2026-09-06** (suppressible alert). One-time sheet "N sessions keep running; `vterm ls` to find them" | — | `06` |
...
| E4 links | **Built 2026-09-06** (hover underline, ⌘-click, `[editor] program` with per-editor `line:col` forms; Dock drop and the Finder service not yet). Path/URL detector over line text (`session.text`) on hover; `⌘`-click opens URL, or file in `[editor] program` (`$EDITOR`, `code`, `zed`, JetBrains `idea`, …) with `line:col` forms; Dock drop opens a tab at the folder; Finder service "New Vambiant Term tab here" | detector unit tests on the six suffix forms | `09` |
| E6 bell | **Built 2026-09-06.** `[terminal] bell = none\|sound\|flash`; `TermEvent::Bell` → `NSSound`/flash | — | `09` |
| E7 notifications | **Built 2026-09-06**: `password_prompt` (cursor line asks for a password/passphrase *and* the PTY has echo off — `Pty::echo_off` via `tcgetattr`, reported once per prompt) → the hint line says keys go unechoed, a toast when the pane is off screen, Notification Center in the background; long commands and agent stop/crash were done with G3. Daemon emits `session.event {command_finished, duration}` and `password_prompt` (heuristic on `[Pp]assword:` at the cursor with echo off, via the observer); app `UNUserNotificationCenter` when not frontmost, `[notifications] long_running_threshold_s = 30`, sound, toast duration; OSC 9/777 already parsed | observer tests | `06 §8`, `09` |
...
| G3 notifications | **Built 2026-09-06** (mailbox with All/Unread/Errors, toasts max 2 with hover pause, `✋ n` sidebar badge for approvals, kinds complete/request/error/info; docs/06 §8 as-built). Inbox history becomes the **mailbox** (`06 §3`): All/Unread/Errors, `↑↓ ↩ ⇧Tab Esc`; toasts (max 2, hover pauses) for other-tab events; tab/sidebar badges; event kinds complete/request/error from adapters | Swift model tests; e2e events | `06 §3`, `06 §8` |
...
| G11 events | **First slice 2026-09-06**: the app folds `agent.event` into an activity strip above the pane's input (tool call in flight with a one-line summary, last result with ✔/✕ and duration, `TodoWrite` progress `n/m tasks · ● current`, thinking, last assistant line) and a click opens the event log (last 200 lines, files touched). Not yet: `AskUserQuestion` card, plan documents, compaction markers, the prompt queue. Render `TodoWrite` (task list ● ✔ ○ ■), plan documents, `AskUserQuestion` (numbered options card answering into the inbox), thinking (collapsed), compaction markers; prompt queue per session: `session.queue {id, text}` sent when state becomes `idle`/`awaiting_input`, panel with reorder/edit/delete, `↩` on empty input sends next | adapter fixture replays; e2e queue | `03`, `06 §4` |
...
| 7 | M9 polish | C9, C11, C12, C15, ~~D1 themes import incl. Warp YAML~~ (built 2026-09-06: Warp, Ghostty, Alacritty, iTerm2, base16), D2 text options, E10 a11y, JSON schema | 3 | unchanged |
