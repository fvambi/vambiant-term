...
| D2 Agent Mode | **First slice built 2026-09-06** (docs/06 §6 as-built): conversation panel, `⌘↩`/`⌘K`/`⌥E` → `ai.ask` with history, cost/token meters, staged commands. Next: streaming, tool loop under `vt-policy` with the approval card, thinking rows, task ticks | Swift model tests; `ai_e2e` history | `06 §6`, `07` |
