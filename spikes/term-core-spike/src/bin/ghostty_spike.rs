...
fn new_term(cols: u16, rows: u16) -> Terminal<'static, 'static> {
    Terminal::new(Options { cols, rows, max_scrollback: 10_000 }).expect("ghostty terminal")
}
...
fn snapshot(term: &Terminal<'static, 'static>) -> (Dirty, usize, String, u64) {
    ...
    while let Some(row) = it.next() {
        ...
        while let Some(cell) = cit.next() {
            ...
            line.push(ch);
            checksum = checksum.wrapping_mul(31).wrapping_add(u64::from(cp.max(0x20)));
        }
...
fn spike_pty() {
    ...
    term.resize(80, 10, 8, 16).expect("initial resize sets cell px");
    let mut chunk_no = 0;
    drain_pty(&mut pty, |bytes| {
        chunk_no += 1;
        term.vt_write(bytes);
        let (dirty, dirty_rows, _, _) = snapshot(&term);
        println!("[chunk {chunk_no} ({} B)] dirty: {dirty:?} rows={dirty_rows}", bytes.len());
    });
    ...
    println!("[after resize 40x10] dirty: {dirty:?} rows={rows}\n--- grid at 40x10 (reflowed) ---\n{text}");

    term.resize(80, 10, 8, 16).expect("resize");
    let (dirty, rows, text, _) = snapshot(&term);
    println!("[after resize 80x10] dirty: {dirty:?} rows={rows}\n--- grid at 80x10 (reflowed back) ---\n{text}");
    println!(
